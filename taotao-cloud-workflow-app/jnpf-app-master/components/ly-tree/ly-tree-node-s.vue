	...
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{ 
		...
		<view class="ly-tree-node__content" :class="{
			...
			<view class="ly-tree-node__label_box">
				<text class="ly-tree-node__label">{{node.data.userName}}</text>
				<text
					class="ly-tree-node__label">{{node.data.position ? node.data.department +'/'+node.data.position : node.data.department}}</text>
			</view>
