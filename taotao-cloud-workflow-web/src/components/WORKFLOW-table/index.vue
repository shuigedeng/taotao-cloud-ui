<template>
  <div class="tableContainer">
    <el-table v-if="!refreshTable" :data="data" ref="WORKFLOWTable" class="WORKFLOW-common-table"
      ...
      <el-table-column prop="index" type="index" width="50" label="序号" v-if="hasNO"
        align="center" />
      <workflow-table-column :columns="columns" :columnList="columnList" v-if="customColumn" />
      <template v-else>
        <slot />
      </template>
...
export default {
  name: 'WORKFLOW-table',
  components: { WorkflowTableColumn, ColumnSettings },
  props: {
    data: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    getColumns() {
      ...
      const cacheList = this.workflow.storageGet(this.menuId)
      if (!cacheList) {
    ...
    doLayout() {
      setTimeout(() => {
        this.$nextTick(() => {
          this.$refs && this.$refs.WORKFLOWTable && this.$refs.WORKFLOWTable.doLayout()
        })
      }, 50)
    },
    setColumn(list) {
      // this.loading = true
      this.workflow.storageSet({ [this.menuId]: list })
