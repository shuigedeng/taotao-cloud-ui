		...
		<view class="flowBefore-actions" v-if="tabIndex != 4 && tabIndex != 3">
			...
			<template v-if="config.opType == 0">
				<template v-if="config.status == 1">
					...
					<u-button class="buttom-btn" type="error" @click.stop="operate('revoke','撤回')"
				...
				<u-button class="buttom-btn" @click.stop="workflow.goBack()" v-else>返回</u-button>
			</template>
			...
			<u-button class="buttom-btn" @click.stop="workflow.goBack()" v-if="config.opType == 3">返回</u-button>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			if (!config) return this.workflow.goBack()
			this.config = config
			this.init()
			uni.$on('operate', data => {
