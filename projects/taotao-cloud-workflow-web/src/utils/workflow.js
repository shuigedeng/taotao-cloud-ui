...
const workflow = {
  ...
  getDate(format, strInterval, number) {
    ...
    return workflow.toDate(myDate, format);
  },
  toDate(v, format) {
    ...
    if (childNode.length > 0) {
      for (let i = 0; i < childNode.length; i++) {
        let treeModel = {
          ...
          ChildNodes: workflow.toTreeViewJson(
            data,
            childNode[i][idText],
            parentIdText,
            idText
  ...
  toFileSize(size) {
    ...
    if (size < 1024.0) return workflow.toDecimal(size) + " 字节";
    else if (size >= 1024.0 && size < 1048576)
      return workflow.toDecimal(size / 1024.0) + " KB";
    else if (size >= 1048576 && size < 1073741824)
      return workflow.toDecimal(size / 1024.0 / 1024.0) + " MB";
    else if (size >= 1073741824)
      return workflow.toDecimal(size / 1024.0 / 1024.0 / 1024.0) + " GB";
  },
  toDecimal(num) {
    if (num == null) {
      num = "0";
