<template>
  <el-select v-model="valueTitle" :clearable="clearable" :disabled="disabled" @clear="clearHandle"
    ref='elSelect' :placeholder="placeholder" :popper-class="`WORKFLOW-select-tree ${themeClass}`"
    @focus="selectFocus" :filterable="filterable" :filter-method="selectFilter"
    class="WORKFLOW-selectTree" @visible-change="visibleChange" :multiple="multiple"
...
export default {
  ...
  methods: {
    ...
    initScroll() {
      this.$nextTick(() => {
        let list = document.querySelectorAll('.WORKFLOW-select-tree .el-scrollbar .el-select-dropdown__wrap')
        // 有多个下拉框，造成页面错位
