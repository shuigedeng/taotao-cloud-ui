		...
		<u-form :model="dataForm" :rules="rules" ref="dataForm" :errorType="['toast']" label-position="left"
			...
			<u-form-item label="开关" prop="switch">
				<view class="u-flex u-form-item-switch">
					<workflow-switch v-model="dataForm.switch"></workflow-switch>
				</view>
			</u-form-item>
			<!-- 单选框组 -->
			...
			<u-form-item label="多选框组" prop="checkbox">
				<workflow-checkbox v-model="dataForm.checkbox" :options="checkboxList"></workflow-checkbox>
			</u-form-item>
			<!-- 下拉框组 -->
			<u-form-item label="下拉框组" prop="select">
				<workflow-select v-model="dataForm.select" placeholder="请选择下拉框组" :options="checkboxList"></workflow-select>
			</u-form-item>
			<!-- 下拉多选 -->
			<u-form-item label="下拉多选" prop="select1">
				<workflow-select v-model="dataForm.select1" placeholder="请选择下拉框组" :options="checkboxList" multiple>
				</workflow-select>
			</u-form-item>
			<!-- 级联选择 -->
			<u-form-item label="级联选择" prop="cascader">
				<workflow-cascader v-model="dataForm.cascader" placeholder="请选择级联选择" :options="cascaderList">
				</workflow-cascader>
			</u-form-item>
			<!-- 省市区 -->
			<u-form-item label="省市区" prop="address">
				<workflow-city-select v-model="dataForm.address" placeholder="请选择省市区" :level="2">
				</workflow-city-select>
			</u-form-item>
			<!-- 树形选择 -->
			<u-form-item label="树形选择" prop="treeSelect">
				<workflow-tree-select v-model="dataForm.treeSelect" placeholder="请选择树形选择" :options="treeData">
				</workflow-tree-select>
			</u-form-item>
			<!-- 文本 -->
			<workflow-text v-model="text"></workflow-text>
			<!-- 上传图片 -->
			<u-form-item label="上传图片" prop="uploadImg">
				<workflow-upload v-model="dataForm.uploadImg"></workflow-upload>
			</u-form-item>
			<!-- 富文本 -->
			<!-- <u-form-item label-width="0" prop="editor">
				<workflow-editor v-model="dataForm.editor" disabled></workflow-editor>
			</u-form-item> -->
			<!-- 评分 -->
			...
			<view class="workflow-card">
				...
				<u-form-item label="时间选择" prop="time">
					<workflow-date-time type="time" v-model="dataForm.time"></workflow-date-time>
				</u-form-item>
				<!-- 日期选择 -->
				<u-form-item label="日期选择" prop="date">
					<workflow-date-time type="date" v-model="dataForm.date"></workflow-date-time>
				</u-form-item>
			</view>
			<view class="workflow-card">
				<!-- 时间选择 -->
				...
				<!-- 日期选择 -->
				<u-form-item label="日期选择" prop="date">
					<workflow-date-time type="datetime" v-model="dataForm.date"></workflow-date-time>
				</u-form-item>
			</view>
			<!-- 公司组件 -->
			<u-form-item label="公司组件" prop="comSelect">
				<workflow-com-select v-model="dataForm.comSelect" placeholder="请选择">
				</workflow-com-select>
			</u-form-item>
			<!-- 部门组件 -->
			<u-form-item label="部门组件" prop="depSelect">
				<workflow-org-select type="department" v-model="dataForm.depSelect"></workflow-org-select>
			</u-form-item>
			<!-- 岗位组件 -->
			<u-form-item label="岗位组件" prop="posSelect">
				<workflow-org-select type="position" v-model="dataForm.posSelect"></workflow-org-select>
			</u-form-item>
			<!-- 用户组件 -->
			<u-form-item label="用户组件" prop="userSelect">
				<workflow-org-select v-model="dataForm.userSelect"></workflow-org-select>
			</u-form-item>
			<!-- 分组标题 -->
			<workflow-group content="分组标题" content-position="right"></workflow-group>
			<!-- 系统控件(7个) + 单据组件 -->
			...
			<view class="workflow-table">
				<view class="workflow-table-item" v-for="(item,i) in dataForm.table" :key="i">
					<view class="workflow-table-item-title u-flex u-row-between">
						<text class="workflow-table-item-title-num">设计子表({{i+1}})</text>
						<view class="workflow-table-item-title-action" v-if="dataForm.table.length>1" @click="delItem(i)">删除
						</view>
					</view>
				...
				<view class="workflow-table-addBtn" @click="addItem">
					<u-icon name="plus" color="#2979ff"></u-icon>添加
				</view>
			</view>
		</u-form>
		<view class="buttom-actions">
			<u-button class="buttom-btn" @click="workflow.goBack">取消</u-button>
