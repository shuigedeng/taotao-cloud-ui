<template>
	<u-form :model="formData" :rules="rules" ref="dataForm" :errorType="['toast']"
		...
		:label-align="formConf.labelPosition==='right'?'right':'left'" class="workflow-form">
		...
		methods: {
			initFormData(componentList, formData, tableRefs) {
				...
				componentList.forEach(cur => {
					...
					if (cur.__vModel__) {
						...
						if (config.children && cur.__config__.workflowKey !== 'table') {
							this.initFormData(config.children, formData)
						}
						if (cur.__config__.workflowKey == 'table' && !cur.__config__.noShow) {
			...
			buildRules(componentList, rules) {
				componentList.forEach(cur => {
					...
					const rule = config.regList.map(item => {
						...
						if (useNumList.includes(workflowKey)) item.type = 'number'
						if (useArrayList.includes(workflowKey) && cur.multiple) item.type = 'array'
						if (Array.isArray(config.defaultValue)) item.type = 'array'
						return item
					})
					if (rule.length) rules[cur.__vModel__] = rule
					if (config.children && workflowKey !== 'table') this.buildRules(config.children, rules)
				})
			},
			setValue(item) {
				if (item.__vModel__) {
			...
			onChange(item) {
				if (item && item.on && item.on.change) {
					const func = this.workflow.getScriptFunc(item.on.change);
					if (!func) return
					func.call(this, {
			...
			onBlur(item, data) {
				...
				if (item && item.on && item.on.blur) {
					const func = this.workflow.getScriptFunc(item.on.blur);
					if (!func) return
					func.call(this, {
						data: data,
						...this.parameter
			...
			onTabChange(item, data) {
				if (item && item.on && item.on['tab-click']) {
					const func = this.workflow.getScriptFunc(item.on['tab-click']);
					if (!func) return
					func.call(this, {
						data: data,
						...this.parameter
			...
			onButtonClick(item, event) {
				if (item && item.on && item.on.click) {
					const func = this.workflow.getScriptFunc(item.on.click);
					if (!func) return
					func.call(this, {
						data: event,
						...this.parameter
			...
			onLoadFunc(formConfCopy) {
				...
				const onLoadFunc = this.workflow.getScriptFunc(formConfCopy.funcs.onLoad)
			...
			beforeSubmit() {
				if (!this.formConfCopy || !this.formConfCopy.funcs || !this.formConfCopy.funcs.beforeSubmit) return Promise
					.resolve()
				const func = this.workflow.getScriptFunc(this.formConfCopy.funcs.beforeSubmit)
			...
			afterSubmit() {
				...
				const func = this.workflow.getScriptFunc(this.formConfCopy.funcs.afterSubmit)
				if (!func) return
				func(this.parameter)
			},
			checkTableData() {
